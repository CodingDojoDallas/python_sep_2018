...
def insert():
    mysql = connectToMySQL("emails")
    query = "INSERT INTO users (email_address, password, first_name, last_name) VALUES (%(email)s, %(password)s, %(first_name)s, %(last_name)s);"
    data = {
...
def success():
    mysql = connectToMySQL("emails")
    query = "SELECT messages.content, users.first_name, users.last_name,messages.user_id, messages.receiver_id, receivers.first_name,receivers.last_name,receivers.id, receivers.email_address FROM users JOIN messages ON users.id = messages.user_id JOIN users AS receivers ON messages.receiver_id = receivers.id WHERE receivers.id = %(id)s"
    data = {
        'id': session['id']
    }
    ...
    print(emails)
    print(len(emails))
    session['message_total'] = len(emails)
