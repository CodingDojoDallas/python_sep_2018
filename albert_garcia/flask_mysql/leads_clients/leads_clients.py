...
def Index():
    ...
    all_clients = mysql.query_db("select concat_ws(' ', clients.first_name, clients.last_name) as client_name, count(leads.clients_client_id) as number_of_leads from leads join clients on leads.clients_client_id = clients.client_id group by leads.clients_client_id;")
    length = len(all_clients)
    print(length)
    print("Fetched all clients", all_clients)
    mysql = connectToMySQL("leads_clients")
    chart = mysql.query_db("select count(leads.clients_client_id) as number_of_leads, concat_ws(' ', clients.first_name, clients.last_name) as client_name from leads join clients on leads.clients_client_id = clients.client_id group by leads.clients_client_id;")
    return render_template('leads_clients.html', clients = all_clients, length = length, chart = chart)
